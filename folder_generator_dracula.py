...
class UniversalFolderCreator:
    ...
    def _run_import_task(self):
        ...
        base_root = "/Users/mac/Pictures/工作"
        ...
        source_vr = "/Volumes/Osmo360/DCIM"
        ...
        for res in results:
            if res:
                r_logs, r_errors, r_moved = res
                all_logs.extend(r_logs)
                all_errors.extend(r_errors)
                total_moved += r_moved
        
        # 结果反馈
        self.master.after(0, self._show_import_result, all_logs, all_errors, total_moved)
    ...
    def _process_single_task(self, task, results, index):
        ...
        if not os.path.exists(src_dir):
            ...
            results[index] = (logs, errors, moved_count)
            return
        ...
        try:
            ...
            for idx, filename in enumerate(files):
                ...
                speed_str = ""
                if elapsed_time > 0:
                    speed_mbps = (total_bytes_transferred / (1024 * 1024)) / elapsed_time
                    speed_str = f"{speed_mbps:.1f} MB/s"
                
                # 更新进度
                current_val = idx + 1
                self.master.after(0, self._update_progress_ui, pbar, plabel, prefix, current_val, total_files, speed_str)
    ...
    def _update_progress_ui(self, pbar, plabel, prefix, current, total, speed=""):
        percent = int((current / total) * 100)
        pbar['value'] = current
        speed_text = f" - {speed}" if speed else ""
        plabel.config(text=f"{prefix}: {percent}% ({current}/{total}){speed_text}")

    def _show_import_result(self, logs, errors, moved_count):
        ...
        if errors:
            result_msg += "\n\n❌ 错误列表:\n" + "\n".join(errors)
            messagebox.showwarning("导卡完成 (有错误)", result_msg)
        else:
            if moved_count > 0:
                messagebox.showinfo("导卡成功", f"共移动 {moved_count} 个文件！\n\n{result_msg}")
            else:
                messagebox.showinfo("导卡结束", f"未发现可移动的文件。\n\n{result_msg}")
                
        self.status_bar.config(text=f"导卡结束，共移动 {moved_count} 个文件")
